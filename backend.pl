...
require "$realpath";
	...
	$fth = $dbh->prepare("INSERT INTO Graphs (id,IP,PORT,COMMUNITY) SELECT DEVICES.id, DEVICES.IP, DEVICES.PORT, DEVICES.COMMUNITY FROM DEVICES ON DUPLICATE KEY UPDATE interfaces=0,name=0,descr=0,sysname=0;");		
...
sub cback3
{
	...
	foreach (keys (%filter))
  {
	 ...
	 $det4 = $filter{"$ip:$ports:$com"}{details}{'1.3.6.1.2.1.2.2.1.2.'.$_};
	 $ifn_st = $_.".".$ifn;
	 $det_st = $_.".".$det4;
	 push @ifname,$ifn_st;
	 push @details,$det_st;
	...
	sub cback5
	{
	 ...
	 foreach (keys (%{$session->var_bind_list()}))
		{
		 $filter{"$ip:$ports:$com"}{details2}{$_} = $session->var_bind_list()->{$_};				
		}	
